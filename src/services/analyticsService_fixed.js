...
class AnalyticsService {
  ...
  /**
   * Get historical AQI data (required by useAnalyticsData hook)
   */
  async getHistoricalAQI(latitude, longitude, days = 7) {
    const location = { latitude, longitude };
    return await this.getHistoricalAnalytics(days, location);
  }

  /**
   * Get personal insights (required by useAnalyticsData hook)
   */
  async getPersonalInsights(latitude, longitude) {
    const cacheKey = `insights-${latitude}-${longitude}`;
  ...
  generateMockHistoricalData(days = 7, location = null) {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const pm25 = Math.round(baseAQI * 0.6 + Math.random() * 20);
      const pm10 = Math.round(baseAQI * 0.8 + Math.random() * 25);
      const no2 = Math.round(Math.max(5, baseAQI * 0.3 + Math.random() * 15));
      const o3 = Math.round(Math.max(10, baseAQI * 0.4 + Math.random() * 20));
      const co = Math.round(Math.max(1, baseAQI * 0.1 + Math.random() * 5));
      const so2 = Math.round(Math.max(2, baseAQI * 0.2 + Math.random() * 8));
      ...
      historical.push({
        ...
        co,
        so2,
        location: location ? `${location.latitude.toFixed(2)}, ${location.longitude.toFixed(2)}` : 'Default'
      });
    }
    ...
    return {
      ...
      summary: {
        ...
        averageAQI: Math.round(historical.reduce((sum, item) => sum + item.aqi, 0) / historical.length),
        maxAQI: Math.max(...historical.map(item => item.aqi)),
        minAQI: Math.min(...historical.map(item => item.aqi))
      }
    };
  }
  ...
  generateMockWeeklyData(location = null) {
    ...
    for (let i = 0; i < 7; i++) {
      const baseAQI = 45 + Math.sin(i * 0.7) * 20 + Math.random() * 30;
      thisWeek.push({
        day: i,
        aqi: Math.round(baseAQI),
    ...
    for (let i = 0; i < 7; i++) {
      const baseAQI = 50 + Math.sin(i * 0.8) * 25 + Math.random() * 35;
      lastWeek.push({
        day: i,
        aqi: Math.round(baseAQI),
    ...
    const thisWeekAvg = Math.round(thisWeek.reduce((sum, item) => sum + item.aqi, 0) / 7);
    const lastWeekAvg = Math.round(lastWeek.reduce((sum, item) => sum + item.aqi, 0) / 7);
    const change = ((thisWeekAvg - lastWeekAvg) / lastWeekAvg) * 100;
    
    return {
      thisWeek: {
        average: thisWeekAvg,
        bestDay: Math.min(...thisWeek.map(d => d.aqi)),
        worstDay: Math.max(...thisWeek.map(d => d.aqi)),
        data: thisWeek
      },
      lastWeek: {
        average: lastWeekAvg,
        bestDay: Math.min(...lastWeek.map(d => d.aqi)),
        worstDay: Math.max(...lastWeek.map(d => d.aqi)),
        data: lastWeek
      },
      change: {
